...
- `pnpm pack:release` 用 esbuild 打出单文件 CLI + `profiles/` + `catalog/`
- npm 包名 `@harness/cli`，bin 为 `harness`；`harness --version` 打印协议版本
- 从源码目录 walk 或从 bundle `dist/` walk 都能找到 `profiles/standard.yml`
- tag `v*` 时 GitHub Actions 上传 tarball
- 协议 0.26.0
