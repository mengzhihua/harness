...
test("policy allows workspace writes and denies secrets", () => {
  ...
  assert.equal(p.decide({ name: "list_dir", args: { path: "." }, deny: false }).verdict, "allow");
  assert.equal(p.decide({ name: "move_file", args: { from: "a.js", to: "b.js" }, deny: false }).verdict, "allow");
...
test("ask mode cannot write", () => {
  ...
  assert.equal(p.decide({ name: "move_file", args: { from: "a.js", to: "b.js" }, deny: false }).verdict, "deny");
  assert.equal(p.decide({ name: "list_dir", args: {}, deny: false }).verdict, "allow");
