...
> **P7 dogfood 切片可启动。** 决策以 [docs/decisions.md](docs/decisions.md) 为准。组合内核对齐 Cordis；Spring 只作理念对照。
...
默认 `harness` 在 TTY 下进 TUI（流 + 当前工具 + 输入；审批卡片 `y/n/s`），非 TTY 仍是 REPL。`harness exec` 结束时打印 `.traj` 路径。
...
进仓库根目录直接 `pnpm harness` 进入 TUI（非 TTY 则 REPL：`/ask` `/plan` `/agent` `/fusion` `/steer` `/plugins` `/traj` `/apply` `/undo` `/quit`）。
